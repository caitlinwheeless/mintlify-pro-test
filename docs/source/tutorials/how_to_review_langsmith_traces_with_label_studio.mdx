---
title: How to Review LangSmith Traces with Label Studio
description: Learn how to pull LangSmith LLM traces into Label Studio Enterprise and annotate them with a custom ReactCode UI.
duration: 10-15 mins
---
...
```python
!pip -q install requests label-studio-sdk python-dotenv langsmith langchain langchain-anthropic anthropic
```
...
**LangSmith Setup**: Visit [LangSmith Documentation](https://docs.langchain.com/langsmith/home) to create an account, get your API key, and set up a project.
...
- LangSmith auto-instruments all LangChain/LangGraph calls when `LANGSMITH_TRACING=true` — no manual callbacks needed
- Engineer-centric interface for technical debugging and iteration
- Hierarchical run storage: each trace is a tree of typed runs (`llm`, `tool`, `chain`)
...
>
> <a href="https://app.humansignal.com/b/MTY1Mg==?p=e"
...
<div style={{textAlign: 'center', margin: '20px 0'}}>
  <img src="https://hs-sandbox-pub.s3.us-east-1.amazonaws.com/blogs-draft/tutorial-llm-reactcode-langsmith.png" alt="ReactCode 3-panel trace review UI for Langsmith" style={{maxWidth: '1000px', width: '100%', height: 'auto', border: '1px solid #ddd', borderRadius: '8px'}} />
</div>
...
- **Braintrust / Langfuse**: See companion tutorials for other observability platforms

## Summary

This tutorial demonstrated the complete workflow from LangSmith traces to expert evaluation:

1. ✓ Set up environment with LangSmith and Label Studio Enterprise
2. ✓ Defined a ReactCode-based 3-panel annotation UI (Enterprise feature)
3. ✓ Ran a multi-tool ReAct agent with Claude extended thinking — LangSmith captured traces automatically via `LANGSMITH_TRACING=true`
4. ✓ Fetched traces from LangSmith using the `langsmith` SDK
5. ✓ Normalized the hierarchical run structure into a unified trace schema
...
- [Companion tutorials: Braintrust](how_to_review_braintrust_traces_with_label_studio) | [Langfuse](how_to_review_langfuse_traces_with_label_studio)
