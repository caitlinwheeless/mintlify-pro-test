---
title: Access tokens
description: Access tokens to interact with the Label Studio API and SDK. 
...
<thead>
  ...
  <td>
    <ul>
        <li>Have a TTL that can be set at the org level. (Label Studio Enterprise only)</li>
        <li>Are only visible to users once</li>
        <li>JWT refresh tokens</li>
        <li>Can be manually revoked</li>
        <li>Require <a href="#HTTP-API">extra steps</a> when used with HTTP API</li>
        <li> <code>-H 'Authorization: Bearer &lt;token&gt;'</code> with HTTP API requests</li>
        <li>Only need to be set once when used with the SDK</li>
    </ul>
  </td>
  <td>
    <ul>
        <li>Does not expire</li>
        <li>Remains listed and available in your account settings</li>
        <li>Can be manually revoked</li>
        <li>Do not need to be refreshed with used with HTTP API</li>
        <li> <code>-H 'Authorization: Token  &lt;token&gt;'</code> with HTTP API requests</li>
        <li>Only need to be set once when used with the SDK</li>
    </ul>
  </td>
  </tr>
...
<img src="/images/admin/token-settings.png" style={{maxWidth: '668px'}} alt="Screenshot of Access Token window" />

</div>

<div className="opensource-only">

<img src="/images/admin/token-settings-lso.png" style={{maxWidth: '475px'}} alt="Screenshot of Access Token window" />
