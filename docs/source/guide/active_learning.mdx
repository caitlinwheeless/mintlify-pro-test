...
description: Set up an end-to-end active learning loop with Label Studio using the ML backend SDK and webhooks to perform model training and predictions and labeling.
sidebarTitle: Active learning loop
---
...
<br/><img src="/images/LS-active-learning.png" alt="Diagram of the active learning workflow described in surrounding text" className="gif-border" width="800px" height="472px" />
...
For more details on the webhook event payloads, see the full [payload details for the annotation webhook](webhook_reference#Annotation-Created). 
...
- If you want to delete all predictions after your model is retrained, see how to [delete predictions](ml#Delete-predictions).
- If you need to retrieve and save predictions for all tasks, see the recommendations for [retrieving predictions from a model](ml#Get-predictions-from-a-model).
...
1. Manually [retrieve predictions from a model](ml#Get-predictions-from-a-model).
2. [Sort the tasks in the data manager by prediction score](manage_data#Example-Sort-by-prediction-score).
