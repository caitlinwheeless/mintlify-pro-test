...
description: Label Studio reference documentation for webhook event fields and payloads sent from Label Studio for integration with your machine learning pipeline. 
sidebarTitle: Webhook event reference
parent: webhooks
parent_enterprise: webhooks
---

Label Studio includes several types of webhook events that trigger when specific actions occur. Refer to the details on this page to determine what information is included in the payload of each webhook event. For details about how to use webhooks and to set up webhooks in Label Studio, see [Set up webhooks in Label Studio](webhooks). To create your own webhooks in Label Studio to trigger events when specific actions happen, see [Create custom events for webhooks in Label Studio](webhook_create). 
...
The HTTP POST payloads that Label Studio sends to the configured webhook URLs include the headers that you set up when you [configure the webhook](webhooks).
...
Sent when a task is created in Label Studio. See how to [set up a webhook for this event](webhooks). 
...
<br/>
<Accordion title="Click to expand the example payload">
```json
{
...
Sent when a task is deleted from Label Studio. See how to [set up a webhook for this event](webhooks).
...
## Annotation Created
Sent when an annotation is created for a task in Label Studio. See how to [set up a webhook for this event](webhooks).
...
Sent when an annotation is updated. See how to [set up a webhook for this event](webhooks).
...
## Annotation Deleted
Sent when an annotation is deleted. See how to [set up a webhook for this event](webhooks).
...
Sent when a project is created. See how to [set up a webhook for this event](webhooks).

You must [enable organization-level webhooks](webhooks#Enable-organization-level-webhooks) to use this event.
...
## Project Updated
Sent when a project is updated. See how to [set up a webhook for this event](webhooks).
...
## Project Deleted
Sent when a project is deleted. See how to [set up a webhook for this event](webhooks).
