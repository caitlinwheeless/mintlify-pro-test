...
description: Label Studio Enterprise documentation for setting up SCIM2
sidebarTitle: SCIM2
parent: admin_auth
...
* SCIM interacts with your SSO integration. Before you begin, you must have SSO already configured. If you do not have SSO set up yet, then follow [Set up SSO](auth_setup).
...
3. Under **Configure SAML**, set up the SAML integration following the steps outlined in [Set up SSO guide](auth_setup).
...
<Warning title="Unsupported attributes cause provisioning errors">
