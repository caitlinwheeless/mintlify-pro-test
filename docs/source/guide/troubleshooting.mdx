...
description: Troubleshooting issues in Label Studio Community Edition
sidebarTitle: Troubleshooting Label Studio
---

import ImageUnits from "/docs/source/includes/image_units.mdx";
...
<Accordion title="Click for details">
```conf
...
<Accordion title="Google Cloud Storage credentials">
...
<Accordion title="Amazon S3 credentials">
...
- If you're placing JSON files in [cloud storage](storage), ensure that if you have multiple tasks in the same file, they are all formatted the same way (for example, you cannot have 1 task with the raw contents of the `data` field and another task that contains annotations and predictions in the same file).
...
<Accordion title="Image annotation units">
<ImageUnits />
</Accordion>
...
To prevent the HTML files from being minified, you can use a different import method. See [Import HTML data](tasks#Import-HTML-data) for more.
...
- Check to see whether the ML backend predictions format follows the same structure as [predictions in imported pre-annotations](predictions).
