...
description: Deploy Label Studio on Kubernetes, such as on Amazon Elastic Container Service for Kubernetes, to create machine learning and data science projects in a scalable containerized environment.
...
4. (Optional) Set up [persistent storage](persistent_storage).
5. (Optional) Configure [ingress](ingress_config).
6. (Optional) Configure [values.yaml](helm_values).
...
If you use a proxy to access the internet from your Kubernetes cluster, or it is airgapped from the internet, see how to [Install Label Studio without public internet access](/guide/install_k8s_airgapped).
