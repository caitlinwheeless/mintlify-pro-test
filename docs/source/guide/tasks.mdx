...
- If your data is stored in a cloud storage bucket, see [Sync data from cloud or database storage](storage).
- If your data is stored in a Redis database, see [Sync data from cloud or database storage](storage).
...
- If your data contains predictions or pre-annotations, see [Import pre-annotated data into Label Studio](predictions).
...
<br/>
<Accordion title="Click to expand example configurations with each valueType">
...
There are several steps to retrieve the data to display in the `Object` tag. The data retrieval is also used in [dynamic choices](/templates/serp_ranking) and [labels](/templates/inventory_tracking). Use the following parameters in the `Object` tag.
...
Use this parameter to retrieve data from multi-column csv on [S3 or other cloud storage](/guide/storage). Label Studio can retrieve it only in run-time, so it's secure.
...
In the `data` JSON dictionary, use key-value pairs that correspond to the source key expected by the object tag in the [label configuration](setup#Customize-the-labeling-interface-for-your-project) that you set up for your project. 
...
| --- | --- | 
| annotations | Optional. List of annotations exported from Label Studio. [Label Studio's annotation format](export#Raw-JSON-format-of-completed-tasks) allows you to import annotation results in order to use them in subsequent labeling tasks. |
| predictions | Optional. List of model prediction results, where each result is saved using [Label Studio's prediction format](export#Raw-JSON-format-of-completed-tasks). Import predictions for automatic task pre-labeling and active learning. See [Import predicted labels into Label Studio](predictions) |

See [Relevant JSON property descriptions](export#Relevant-JSON-property-descriptions) in the export documentation for more details about the JSON format of exported tasks.
...
You can place multiple tasks in one JSON file if you're uploading the JSON file using Label Studio Import Dialog only (Data Manager => Import button), or when importing from [cloud storage](storage). When using cloud storage, you must ensure every task in the file is formatted the same way. 

If you're using [Source cloud storage](storage), you can also place multiple tasks in a newline-delimited JSON file (JSONL/NDJSON).

<br/>
<Accordion title="To place multiple tasks in one JSON file, use this JSON format example">
...
<br/>
<Accordion title="For versions of Label Studio earlier than 1.0.0, use this JSON format example.">
...
If you want to import entire plain text files without each line becoming a new labeling task, customize the labeling configuration to specify `valueType="url"` in the Text tag. See the [Text tag documentation](/tags/text). See more about [how to use the valueType field](#How-to-import-your-data).
...
- Import the HTML files as BLOB storage from [external cloud storage such as Amazon S3 or Google Cloud Storage](storage).
...
- Add the file directory as a source or target [local storage](storage#Local-storage) connection in the Label Studio UI.
...
If your labeling configuration supports HyperText or multiple data types, use the Label Studio JSON format to specify the local file locations instead of a `txt` file. See [an example of this format](storage#Tasks-with-local-storage-file-references).
...
<img src="/images/screens/import-button.png" className="img-template-example" title="Import Button in Data Manager" /> 
...
By default, Label Studio expects JSON-formatted tasks using the [Basic Label Studio JSON format](tasks#Basic-Label-Studio-JSON-format). 
