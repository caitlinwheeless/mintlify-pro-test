...
description: "Label Studio Documentation for integrating Amazon AWS S3, Google Cloud Storage, Microsoft Azure, Redis, and local file directories with Label Studio."
sidebarTitle: Add project storage
---
...
Task data synced from cloud storage is not stored in Label Studio. Instead, the data is accessed using presigned URLs. You can also secure access to cloud storage using VPC and IP restrictions for your storage. For details, see [Secure access to cloud storages](security#Secure-access-to-cloud-storages).
...
<img src="/images/source-cloud-storages.png" className="make-intense-zoom" />
...
<img src="/images/source-storages-treat-off.png" className="make-intense-zoom" />

You may put multiple tasks inside the same JSON file, but not mix task formats inside the same file.

<Accordion title="Example with bare tasks">
`task_01.json`
```
{
...
<Accordion title="Example with tasks, annotations and predictions">
...
<Accordion title="Example with JSONL">
`tasks.jsonl`
```
...
<img src="/images/source-storages-treat-on.png" className="make-intense-zoom" />
...
<img src="/images/storages/storage-proxy-presigned.png" style={{maxWidth: '600px', margin: '0 auto'}} alt="Diagram of presigned URL flow" />
...
<img src="/images/storages/storage-proxy.png" style={{maxWidth: '600px', margin: '0 auto'}} alt="Diagram of proxy flow" />
...
<Note title="Note for on-prem deployments">
