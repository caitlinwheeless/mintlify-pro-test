---
title: Introduction
description: Customize the Label Studio labeling interface using XML-like tags for data annotation tasks in machine learning and data science projects.
---
...
- **Object** tags for data types, used to display elements in a task that can be labeled such as [audio](/tags/audio), [HTML](/tags/hypertext), [images](/tags/image), [paragraphs](/tags/paragraphs), [text](/tags/text), and more.
- **Control** tags that you can use to annotate the objects. For example, use [labels](/tags/labels) for semantic and named entity tasks, [choices](/tags/choices) for classification tasks, [textarea](/tags/textarea) for transcription tasks, and more.
- **Visual** tags to modify the visual aspects of the labeling interface, such as by controlling the [view](/tags/view) of specific labeling options, or adding a [header](/tags/header).

## Create a custom labeling configuration

When you [set up your labeling interface](/guide/setup), you can choose to use tags to create a custom labeling interface. When you create a custom labeling interface, you must pay attention to the structure of your task data and set up the configuration accordingly.
...
<br/>
<img src="/images/tag-header-example.png" alt="Screenshot of the labeling interface showing a header, text to be labeled, and checkboxes with Positive, Negative, and Neutral options." className="gif-border" width="748px" height="284px" />

### Styles

You can style the labeling interface using the [Style](/tags/style) tag, but you can also add styles to the [View](/tags/view) tag.
