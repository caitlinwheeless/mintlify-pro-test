---
title: BitmaskLabels
description: Customize Label Studio with bitmask pixel-wise label tags for image segmentation labeling for machine learning and data science projects.
---

import Bitmasklabels from "/docs/source/includes/tags/bitmasklabels.mdx";

The `Bitmask` tag is for pixel-wise image segmentation tasks, and is used in the area where you want to apply a mask or use a brush to draw a region on the image.

`Bitmask` operates on the pixel level and outputs a base64-encoded PNG data URL image with black pixels on transparent background.

Export data example: `data-url:image/png;[base64-encoded-string]`

<video className="Video astro-OQEP7KKB" loop="" playsinline="" autoplay="" muted="" style={{width: '100%'}}>
  <source src="https://cdn.sanity.io/files/mzff2hy8/production/4812f66851a7fd4836e729bc7ccb7e510823af5d.mp4" type="video/mp4" className="astro-OQEP7KKB" />
</video>

<br/>
<br/>

<Note>

You need to set `smoothing="false"` on the `<Image>` tag to be able to work with individual pixels. 
</Note>
Use with the following data types: image.

<Bitmasklabels />

### Example

Basic image segmentation labeling configuration

```html
<View>
  <BitmaskLabels name="labels" toName="image">
    <Label value="Person" />
    <Label value="Animal" />
  </BitmaskLabels>
  <Image name="image" value="$image" smoothing="false" />
</View>
```
