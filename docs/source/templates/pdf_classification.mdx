...
description: Template for classifying PDF data with Label Studio for your machine learning and data science projects.
---

<img src="/images/templates/pdf-classification.png" alt="" className="gif-border" width="552px" height="408px" />
...
Use the [Rating](/tags/rating) control tag to apply a star rating with a scale of 10 to the pdf:
```xml
<Rating name="rating" toName="pdf" maxRating="10" icon="star" size="medium" />
```
  
Use the [Choices](/tags/choices) control tag to present classification options to the annotator:
```xml
<Choices name="choices" choice="single-radio" toName="pdf" showInline="true">
    <Choice value="Important article"/>
    <Choice value="Yellow press"/>
...
Use the [Pdf](/tags/pdf) tag to render an inline version of the PDF data:
```xml
<Pdf name="pdf" value="$pdf"/>
```
