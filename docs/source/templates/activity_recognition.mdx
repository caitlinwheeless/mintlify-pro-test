...
description: Template for labeling human activity recognition in a time series using Label Studio for your machine learning and data science projects.
---

<img src="/images/templates/activity-recognition.png" alt="" className="gif-border" width="552px" height="408px" />
...
Use the [TimeSeriesLabels](/tags/timeserieslabels) control tag to provide a list of labels that annotators can apply to regions on the time series graph:
```xml
    <TimeSeriesLabels name="label" toName="ts">
        <Label value="Run" background="red"/>
        <Label value="Walk" background="green"/>
...
```
The `overviewChannels` parameter in the [TimeSeries](/tags/timeseries) tag specifies the column in the time series data to display on the time series graph as a channel. 
