...
description: Template for performing object detection with rectangular bounding boxes with Label Studio for your machine learning and data science projects.
---
...
<br />
<img src="/images/templates/object-detection-with-bounding-boxes.png" alt="" className="gif-border" width="552px" height="408px" />
...
Use the [Image](/tags/image) object tag to specify the image to label:
```xml
<Image name="image" value="$image"/>
```
  
Use the [RectangleLabels](/tags/rectanglelabels) control tag to add labels and rectangular bounding boxes to your image at the same time. Use the [Label](/tags/label) tag to control the color of the boxes:
```xml
  <RectangleLabels name="label" toName="image">
    <Label value="Airplane" background="green"/>
    <Label value="Car" background="blue"/>
...
```
The `visibleWhen` parameter of the [View](/tags/view) tag hides the description prompt from annotators until a bounding box is selected. 

After the annotator selects a bounding box, the [Header](/tags/header) appears and provides instructions to annotators.

The [TextArea](/tags/textarea) control tag displays an editable text box that applies to the selected bounding box, specified with the `perRegion="true"` parameter. You can also add a `placeholder` parameter to provide suggested text to annotators. 

In addition, you can prompt annotators to provide additional feedback about the content of the bounding box, such as the status of the item in the box, using the [Choices](/tags/choices) tag with the `perRegion` parameter.

## Related tags

- [Image](/tags/image)
- [RectangleLabels](/tags/rectanglelabels)
- [Rectangle](/tags/rectangle)
- [Label](/tags/label)
