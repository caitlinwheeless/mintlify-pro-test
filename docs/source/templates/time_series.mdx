...
description: Template for labeling multivariate and simple time series data with Label Studio for your machine learning and data science projects.
---

<img src="/images/templates/time-series-forecasting.png" alt="" className="gif-border" width="552px" height="408px" />
...
Use the [TimeSeriesLabels](/tags/timeserieslabels) control tag to highlight and label specific spans of a time series graph:
```xml
<TimeSeriesLabels name="label" toName="ts">
    <Label value="Run"/>
    <Label value="Walk"/>
...
```
The tag is linked to the [TimeSeries](/tags/timeseries) object tag with a `toName` parameter.

Use the [TimeSeries](/tags/timeseries) object tag to display time series data and channels:
```xml
<TimeSeries name="ts" valueType="url" value="$csv_url" timeColumn="time">
    <Channel column="sensorone" />
    <Channel column="sensortwo" />
...
- [Channel](/tags/timeseries#Channel)
...
```
In this case, the `<TimeSeries>` tag has the `valueType="url"` attribute because the CSV file is referenced as a URL. See [How to import your data](/guide/tasks#How-to-import-your-data).
...
```
The `value` parameter in the [TimeSeries](/tags/timeseries) tag is used to refer to the JSON key with the CSV file URL. 
